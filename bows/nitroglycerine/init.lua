...
if node.hurt==1 then
	for _, ob in ipairs(minetest.env:get_objects_inside_radius(pos, node.radius*2)) do
...
if node.velocity==1 then
	...
	if node.radius<15 then
		minetest.sound_play("nitroglycerine_explode", {pos=pos, gain = 3.0, max_hear_distance = node.radius*8})
		else
		minetest.sound_play("nitroglycerine_nuke", {pos=pos, gain = 5.0, max_hear_distance = node.radius*30})
	end
end
...
nitroglycerine.freeze=function(ob)
	...
	if ob:get_luaentity().aliveai then
		for _, ob in ipairs(minetest.env:get_objects_inside_radius(pos, 1)) do
